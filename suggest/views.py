...
def page(request, pageindex):
    ...
    # 查询所有的创意
    list1 = Comment.objects.all().order_by("-id")
    # 创建一个paginator对象
...
def show_info(request, id):
    ...
    if user_id:
        temp = 'every_info.html'
    else:
        temp = 'every_info_no.html'

    # 查找创意的对象
    sug = Comment.objects.get(id=int(id))
    # print(sug.title)
    return render(request, temp, {"sug": sug})
...
def select(request, pageindex):
    ...
    if request.method == 'POST':
        ...
        # print(fl_name)
        list1 = Comment.objects.filter(fl=fl_name).order_by('-id')
        # 统计有多少条创意
        count = Comment.objects.filter(fl=fl_name).count()
        # 统计有多少条创意被采纳
        cn_count = Comment.objects.filter(fl=fl_name, sl=True).count()
